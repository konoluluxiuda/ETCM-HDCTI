...
MANIFEST="configs/cold_start_hctx_ablation_manifest.json"
JOBS=(
    "TCM-Suite|tcmsuite|configs/HDCTI_tcmsuite_cold_start_no_context_no_dense_full.conf"
    "TCMSP|tcmsp|configs/HDCTI_tcmsp_cold_start_no_context_no_dense_full.conf"
    "SymMap2.0|symmap|configs/HDCTI_symmap_cold_start_no_context_no_dense_full.conf"
    "ETCM2.0 mention10|etcm_mention10|configs/HDCTI_etcm_mention10_cold_start_no_context_no_dense_full.conf"
)

python tools/validate_cold_start_hctx_ablation_configs.py \
    --manifest "$MANIFEST" || exit 1

if [[ "${1:-}" == "--dry-run" ]]; then
    printf 'Only the following missing NoContext five-fold jobs will run:\n'
    for job in "${JOBS[@]}"; do
...
RUN_DIR="${HDCTI_BATCH_DIR:-$REPOSITORY_ROOT/results/batch_runs/cold_start_hctx_ablation_$RUN_TIMESTAMP}"
...
write_progress_summary() {
    ...
    {
        printf '# Compound cold-start NoContext 补充批处理\n\n'
        printf -- '- 结果目录：`%s`\n' "$RUN_DIR"
        printf -- '- 冻结 manifest：`%s`\n' "$MANIFEST"
        printf -- '- 更新时间：`%s`\n\n' "$(date --iso-8601=seconds)"
        ...
        printf '\n全部完成后，三阶段递进结果写入 `paired/summary.md`。\n'
    } > "$temporary_summary"
...
for job in "${JOBS[@]}"; do
    ...
    printf '%s\tNoContext\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
...
python tools/summarize_cold_start_hctx_ablation.py \
    --manifest "$MANIFEST" \
    --no-context-results "$RESULTS_TSV" \
    --output-dir "$PAIRED_DIR" || exit 1

printf '\nBatch results: %s\n' "$SUMMARY_MD"
printf 'Paired cold-start chain: %s\n' "$PAIRED_DIR/summary.md"
