...
class SupportConditionedExpertsModelSmokeTest(unittest.TestCase):
    def test_warm_and_cold_experts_train_and_share_one_inference_route(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                external_cold_index = model.data.compound['c10']
                self.assertEqual(
                    model.compound_cp_support_degrees[selected_index], 0.0
                )
                np.testing.assert_array_equal(
                    model.supportExpertColdGateValues([
                        selected_index, seen_index, external_cold_index
                    ]),
                    [1.0, 0.0, 1.0],
                )
                ...
                self.assertGreater(
                    float(np.linalg.norm(
                        model.weight['context_herb_protein']
                    )),
                    0.0,
                )
                self.assertGreater(
                    float(np.linalg.norm(
                        model.weight['context_herb_protein_cold']
                    )),
                    0.0,
                )
                ...
                metadata_paths = list(
                    (dataset_dir / 'saved_model').rglob(
                        'support_conditioned_experts.json'
                    )
                )
                ...
                self.assertEqual(metadata['cold_compounds'], 2)
                model.sess.close()
            finally:
