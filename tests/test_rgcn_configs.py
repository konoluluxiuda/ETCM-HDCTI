...
class RGCNConfigsTest(unittest.TestCase):
    CONFIGS = (
        'RGCNCTI_tcmsuite_pair_stratified_pilot.conf',
        'RGCNCTI_tcmsp_pair_stratified_pilot.conf',
        'RGCNCTI_symmap_pair_stratified_pilot.conf',
        'RGCNCTI_etcm_mention10_pair_stratified_pilot.conf',
    )
    ...
    def test_four_dataset_pilots_use_one_frozen_protocol(self):
        ...
        for name in self.CONFIGS:
            ...
            with self.subTest(config=name):
                self.assertEqual(config['model.name'], 'RGCNCTI')
                ...
                self.assertEqual(config['rgcn.layers'], '2')
                self.assertEqual(config['rgcn.activation'], 'relu')
                self.assertEqual(config['rgcn.objective'], 'bce')
