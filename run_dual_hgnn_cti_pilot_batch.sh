...
JOBS=(
    "TCM-Suite|tcmsuite|configs/DualHGNN_tcmsuite_pair_stratified_pilot.conf"
    "TCMSP|tcmsp|configs/DualHGNN_tcmsp_pair_stratified_pilot.conf"
    "SymMap2.0|symmap|configs/DualHGNN_symmap_pair_stratified_pilot.conf"
    "ETCM2.0 mention10|etcm_mention10|configs/DualHGNN_etcm_mention10_pair_stratified_pilot.conf"
)

if [[ "${1:-}" == "--dry-run" ]]; then
    printf 'Frozen Dual-HGNN-CTI single-fold pilots:\n'
    for job in "${JOBS[@]}"; do
...
RUN_DIR="${HDCTI_BATCH_DIR:-$REPOSITORY_ROOT/results/batch_runs/dual_hgnn_cti_pilot_$RUN_TIMESTAMP}"
...
write_summary() {
    ...
    {
        printf '# Dual-HGNN-CTI 四库单折 Pilot\n\n'
        printf -- '- 模型角色：HGHDA-inspired same-input CTI adapter\n'
        printf -- '- 编码器：H-C/P-D 双超图传播\n'
        printf -- '- 已关闭：PageRank、自门控、稠密全节点注意力、节点维度注意力和项目新增模块\n'
...
printf 'All Dual-HGNN-CTI pilot jobs completed successfully.\n'
