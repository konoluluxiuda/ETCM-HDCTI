...
JOBS=(
    "TCM-Suite|tcmsuite|configs/HGTCTI_tcmsuite_pair_stratified_pilot.conf"
    "TCMSP|tcmsp|configs/HGTCTI_tcmsp_pair_stratified_pilot.conf"
    "SymMap2.0|symmap|configs/HGTCTI_symmap_pair_stratified_pilot.conf"
    "ETCM2.0 mention10|etcm_mention10|configs/HGTCTI_etcm_mention10_pair_stratified_pilot.conf"
)

if [[ "${1:-}" == "--dry-run" ]]; then
    printf 'Frozen HGT-CTI single-fold pilots:\n'
    for job in "${JOBS[@]}"; do
...
RUN_DIR="${HDCTI_BATCH_DIR:-$REPOSITORY_ROOT/results/batch_runs/hgt_cti_pilot_$RUN_TIMESTAMP}"
...
write_summary() {
    ...
    {
        printf '# HGT-CTI 四库单折 Pilot\n\n'
        printf -- '- 模型角色：same-input sparse heterogeneous graph transformer baseline\n'
        printf -- '- 图：H-C、inner-train positive C-P、P-D；六类有向关系\n'
        printf -- '- 传播：两层、双头、节点类型投影与关系特异注意力\n'
        printf -- '- 稀疏约束：每种关系对每个目标节点最多保留 64 个确定性入邻居\n'
...
printf 'All HGT-CTI pilot jobs completed successfully.\n'
