...
上述第 18 节完成后，进一步在读取新 outer 指标前预注册 `c1p1` 至 `c4p4`
四个新支持单元。四库共 16 个匹配实验均独立训练 NoContext base 和隔离的
Hctx-P head；只有全部 head、checkpoint 和报告哈希一次性冻结后，才启动 outer
纯推理。

确认结果为 `16/16` 单元 Macro-AUPR 提高，总体平均差值 `+0.105385`，通过
全部预注册 Gate。四库平均差值分别为：

```text
TCM-Suite          +0.048809
TCMSP              +0.137953
SymMap2.0          +0.101750
ETCM2.0-mention10  +0.133030
```

状态平均增量仍遵循固定路由的预期：四库 WW 均小幅提高，CW 分别提高
`+0.177048/+0.544804/+0.384220/+0.523126`，WC/CC 均精确为 `0`。这排除了
此前 `c0p0` 单元偶然有利的主要担忧，并将 V3 升级为四库多支持单元确认性
证据。其能力边界不变：V3 解决的是 cold-compound/warm-target 状态下 base
支持失配，不是通用 target-cold 或 double-cold 模型。

完整协议、逐库均值、标准差和工件哈希见
[重复 Outer 复验](FROZEN_BASE_HCTX_ROUTER_REPEATED_OUTER.md)。确认结果产生后
不得再针对 `c1p1` 至 `c4p4` 修改 head、路由、Gate 或训练超参数。

将历史 `c0p0` 仅作为描述性单元加入后，四库各五单元均保持 `5/5` 正增益，
20 单元总体 Macro-AUPR 增量为 `+0.103993 (±0.035613)`。该五单元结果用于
论文均值、标准差和逐单元展示，不替代只基于 16 个新单元的预注册确认结论。
