...
|---|---:|---:|---:|---|
| TCM-Suite | 0.606038 | 0.647174 | +0.041136 | 不下降 |
| TCMSP | 0.950699 | 0.941207 | -0.009492 | 超过 -0.005，提前终止 |
| SymMap2.0 | 未运行 | 未运行 | - | Gate 已失败 |
| ETCM2.0 | 未运行 | 未运行 | - | Gate 已失败 |

TCM-Suite 基线在 epoch 20 取得最佳 AUPR，SP-FBHA 在 epoch 22 取得最佳 AUPR。候选提高 `0.041136`，运行时间从 `24.03 s` 增至 `27.11 s`（`1.13x`）。该增益远大于普通随机折的 `+0.000490`，并且在成分/蛋白两侧旧稠密注意力均关闭时仍然成立，支持 SP-FBHA 对 compound cold-start 更有针对性的假设。

参数仍主要集中于 H-C node-to-edge：HC-L1/HC-L2 node mean-abs 为 `0.438114/0.648313`；P-D node 参数为 `0.002698/0.000089`。这表明主要信号来自药材超边内部的成分成员选择，而不是 P-D 侧或旧全节点注意力。

TCMSP 基线与候选均在 epoch 50 达到当前最佳 AUPR。SP-FBHA 从 `0.950699` 降至 `0.941207`，下降 `0.009492`，超过预注册最大允许下降 `0.005`；运行时间从 `40.81 s` 增至 `70.16 s`（`1.72x`）。虽然两组都触及 epoch 上限，存在尚未完全收敛的残余不确定性，但不能在观察到失败后通过延长 epoch 改写冻结 Gate。

TCMSP 的 H-C node-to-edge 参数为 `0.388280/0.380060`，P-D node 参数仅为 `0.000459/0.000104`。旧阈值 2000 协议在 TCMSP 保留了蛋白侧 full attention，当时 SP-FBHA 只下降 `0.000940`；统一关闭后下降扩大到 `0.009492`。这提示 TCMSP 的 cold-start 表现依赖蛋白侧全局交互，当前 SP-FBHA 不能独立替代它。

统一 cold-start Gate 因单库下降超过 `0.005` 提前失败，停止 SymMap2.0、ETCM2.0 和完整五折。最终边界为：SP-FBHA 在 TCM-Suite cold-start 上具有明显局部收益，但没有形成跨数据库稳定的冷启动专用模块；普通随机折与统一 cold-start 两条主路线均为 No-Go。保留代码和阳性子结果用于机制讨论，不进入最终主模型，也不搜索单侧开关、先验强度或更长训练轮数。
