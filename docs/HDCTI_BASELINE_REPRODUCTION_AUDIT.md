...
4. `Dual-HGNN-CTI` 代码、四库 pilot 和四库正式五折配置已完成。
5. `LightGCN-CTI` 代码、四库 pilot 和四库正式五折配置已完成。
6. `R-GCN-CTI` 代码、四库 pilot 和四库正式五折配置已完成。
7. 三种外部基线的 12 个任务已接入统一可续跑批处理；下一步运行该冻结批次，
   不在 pilot 或数据库之间调参。
