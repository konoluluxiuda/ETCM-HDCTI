...
| 进行中（实验） | 缺少同一 Strict 协议下的完整外部对比表 | HDCTI 原论文八模型结果可以作为 Legacy 文献比较，但公开材料不含其属性输入、适配代码与共用 fold；Dual-HGNN-CTI、LightGCN-CTI 与 R-GCN-CTI 四库单折 pilot 均已通过，R-GCN AUPR 为 0.993539、0.981896、0.945934、0.974539 | 统一生成 Dual-HGNN/LightGCN/R-GCN 五折配置并运行冻结外层测试，不再根据 pilot 调参；若作者补充材料，再接入可原样复现的属性模型 |
