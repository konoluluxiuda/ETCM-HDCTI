...
| 进行中（实验） | 缺少同一 Strict 协议下的完整外部对比表 | HDCTI 原论文八模型结果可以作为 Legacy 文献比较，但公开材料不含其属性输入、适配代码与共用 fold；Dual-HGNN-CTI、LightGCN-CTI 与 R-GCN-CTI 四库单折 pilot 均已通过，12 个正式五折配置和统一可续跑脚本已冻结 | 运行 `run_external_baselines_full_batch.sh` 完成外层五折测试，不再根据 pilot 调参；若作者补充材料，再接入可原样复现的属性模型 |
