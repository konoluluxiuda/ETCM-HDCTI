...
15 条 E 级候选已在外部检索前冻结，当前唯一优先任务是按清单完成独立证据
核验，不再训练模型。冻结协议和候选见
[ETCM_TOPK_MANUAL_VALIDATION.md](ETCM_TOPK_MANUAL_VALIDATION.md)。

执行顺序：

1. 已从三个模型的 Top-20 中合并 E 级候选并按 `compound-protein` 去重；
2. 已按每个案例 3 条等额分配冻结 15 个身份完整 pair，manifest SHA-256 为
   `b6c68cb0d9e9cc55cd37d773ec099e0b8fc26c73c658609a79774cb78dd5fd01`；
3. 首批 3 条 DEXPROPRANOLOL 候选已完成 BindingDB、ChEMBL、PubMed 检索；
   SIGMAR1 获得 ChEMBL B1 定量结合证据，CA12/MCL1 当前未核验到精确支持；
4. 下一步按相同协议完成余下 12 条候选，继续记录检索日期、查询式、数据库
...
```text
configs/etcm_topk_manual_validation.json（已生成）
...
docs/ETCM_TOPK_MANUAL_VALIDATION.md（已生成）
```
