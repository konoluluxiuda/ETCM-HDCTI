...
6. 在排名冻结后接入原始 Herbs、Targets、Diseases 页面，生成带来源边界的
   post-hoc 上下文与机制路径注释；
7. 完成 15 条冻结 E 级候选的 BindingDB、ChEMBL 和 PubMed 独立核验，
   共执行 45 个预注册查询。
...
15 条 E 级候选已在外部检索前冻结，并已按清单完成独立证据核验。冻结协议、
精确实体身份和逐条来源见
[ETCM_TOPK_MANUAL_VALIDATION.md](ETCM_TOPK_MANUAL_VALIDATION.md)。

| 等级 | Pair 数 | 说明 |
|---|---:|---|
| B1 | 2 | DEXPROPRANOLOL–SIGMAR1；Quercetin–PLAU |
| B2 | 0 | 无仅依靠不完整直接实验支持的 pair |
| D | 0 | 间接证据没有单独升级任何 pair |
| E | 12 | 当前规定来源未核验到精确支持，不是确认负例 |
| Conflict | 1 | Quercetin–OPRD1 的 human binding `AC50 > 30000 nM` |

Quercetin–PLAU 同时具有 human uPA `IC50=12100 nM` 和 X-ray 复合物结构
支持。DEXPROPRANOLOL–SIGMAR1 具有 human radioligand-binding 的
`Ki=1670 nM` 与 `IC50=3974 nM`。这 2 条独立支持来自检索前冻结的 15 条
候选，但 `2/15` 不能解释为总体 precision，因为清单是按五个案例等额选择的
小规模解释样本。

## 11. 代表性案例图

不再扩大数据库检索范围或重新选择候选。页面路径审计发现
DEXPROPRANOLOL–SIGMAR1 和 Quercetin–OPRD1 均没有 C-H-D-P 路径；
Quercetin–PLAU 有 8 条路径，但均来自同一药材 JingDaJi，且只能作为
post-hoc 假设。因此不强行凑足多条机制路径，而是按证据等级冻结三个代表
案例：
...
论文案例表和三面板图已生成：两个 B1 正向案例以及一个 Conflict 失败案例。
图版规范、图注与可复现命令见
[ETCM_REPRESENTATIVE_CASES.md](ETCM_REPRESENTATIVE_CASES.md)。实际图中
已区分直接实验支持、模型侧 Herb 上下文和 H-D 算法/数据库路径假设。
