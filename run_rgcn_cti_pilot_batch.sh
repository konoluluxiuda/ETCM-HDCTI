...
JOBS=(
    "TCM-Suite|tcmsuite|configs/RGCNCTI_tcmsuite_pair_stratified_pilot.conf"
    "TCMSP|tcmsp|configs/RGCNCTI_tcmsp_pair_stratified_pilot.conf"
    "SymMap2.0|symmap|configs/RGCNCTI_symmap_pair_stratified_pilot.conf"
    "ETCM2.0 mention10|etcm_mention10|configs/RGCNCTI_etcm_mention10_pair_stratified_pilot.conf"
)

if [[ "${1:-}" == "--dry-run" ]]; then
    printf 'Frozen R-GCN-CTI single-fold pilots:\n'
    for job in "${JOBS[@]}"; do
...
RUN_DIR="${HDCTI_BATCH_DIR:-$REPOSITORY_ROOT/results/batch_runs/rgcn_cti_pilot_$RUN_TIMESTAMP}"
...
write_summary() {
    ...
    {
        printf '# R-GCN-CTI 四库单折 Pilot\n\n'
        printf -- '- 模型角色：same-input heterogeneous relation graph baseline\n'
        printf -- '- 图：H-C、inner-train positive C-P、P-D；六类有向关系\n'
        printf -- '- 传播：两层 relation-specific mean-normalized aggregation\n'
...
printf 'All R-GCN-CTI pilot jobs completed successfully.\n'
